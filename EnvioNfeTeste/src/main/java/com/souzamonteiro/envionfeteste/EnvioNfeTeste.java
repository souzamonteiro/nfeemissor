...
package com.souzamonteiro.envionfeteste;
...
public class EnvioNfeTeste {
    ...
    private static void Envia() {
        ...
        try {
            ...
            ConfiguracoesNfe config = ConfiguracaoTeste.iniciaConfiguracoes(EstadosEnum.BA, AmbienteEnum.HOMOLOGACAO);

            //Informe o Numero da NFe
            int numeroNfe = 92752;
            //Informe o CNPJ do Emitente da NFe
            String cnpj = "33630582000149";
            ...
            //Informações do Responsável Técnico (OBRIGATÓRIO para AM)
            infNFe.setInfRespTec(preencheResponsavelTecnico());
            
            //Informações do Grupo de Autorização para UF
            infNFe.getAutXML().add(configurarAutorizacaoXML());
    ...
    private static InfNFe.InfIntermed montaInfInterm() {
        ...
        infIntermed.setCNPJ("13937073000156");
        infIntermed.setIdCadIntTran("SEFAZ BAHIA");
        return infIntermed;
    }
    ...
    private static InfNFe.Emit preencheEmitente(ConfiguracoesNfe config, String cnpj) {
        ...
        emit.setCNPJ(cnpj);
        emit.setXNome("Editora Roberto LIEuiz Souza Monteiro");

        TEnderEmi enderEmit = new TEnderEmi();
        enderEmit.setXLgr("Rua Chile");
        enderEmit.setNro("s/n");
        enderEmit.setXCpl("Edifício Eduardo De Moraes, sala 606");
        enderEmit.setXBairro("Centro");
        enderEmit.setCMun("2927408");
        enderEmit.setXMun("Salvador");
        enderEmit.setUF(TUfEmi.valueOf(config.getEstado().toString()));
        enderEmit.setCEP("40020000");
        enderEmit.setCPais("1058");
        enderEmit.setXPais("Brasil");
        enderEmit.setFone("71991721874");
        emit.setEnderEmit(enderEmit);

        emit.setIE("158039716");
        emit.setCRT("3");
    ...
    private static InfNFe.Dest preencheDestinatario() {
        ...
        dest.setCPF("40325635862");
        dest.setXNome("NF-E EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL");

        TEndereco enderDest = new TEndereco();
        enderDest.setXLgr("Rua Tanhaçu");
        enderDest.setNro("11");
        enderDest.setXBairro("Ipitanga");
        enderDest.setCMun("2919207");
        enderDest.setXMun("Lauro de Freitas");
        enderDest.setUF(TUf.valueOf("BA"));
        enderDest.setCEP("42706730");
        enderDest.setCPais("1058");
        enderDest.setXPais("Brasil");
        enderDest.setFone("71991965292");
        dest.setEnderDest(enderDest);
        dest.setEmail("andreiarita2@gmail.com");
        //dest.setIE("");
        dest.setIndIEDest("9");
        return dest;
    }
    ...
    /**
     * Preenche informações do responsável técnico (obrigatório para AM)
     */
    private static TInfRespTec preencheResponsavelTecnico() {
        TInfRespTec infRespTec = new TInfRespTec();

        // CNPJ da empresa ou CPF do responsável técnico
        infRespTec.setCNPJ("09245946000192"); // Substitua pelo CNPJ/CPF real

        // Nome do responsável técnico
        infRespTec.setXContato("CLAUDIO TELES DOS SANTOS");

        // Email do responsável técnico
        infRespTec.setEmail("atendimento@controllercenter.com.br");

        // Telefone do responsável técnico (formato: Código País + Código Área + Número)
        infRespTec.setFone("71999572888");

        // CSRT (Código de Segurança do Responsável Técnico) - se aplicável
        // infRespTec.setIdCSRT("1"); // Opcional

        // Hash CSRT - se aplicável  
        // infRespTec.setHashCSRT("ABCD1234"); // Opcional

        return infRespTec;
    }
    
    /**
     * Preenche informações do Grupo de Autorização para UF
     */
    private static InfNFe.AutXML configurarAutorizacaoXML() {
        InfNFe.AutXML autXML = new InfNFe.AutXML();

        // Autorizar o próprio emitente (sua empresa)
        autXML.setCNPJ("13937073000156");

        // Ou autorizar o destinatário também
        // AutXML autXMLDest = new AutXML();
        // autXMLDest.setCPF("40325635862");
        // infNFe.getAutXML().add(autXMLDest);
